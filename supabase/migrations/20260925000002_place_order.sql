-- place_order: the single path that creates an order.
-- Runs in one transaction, locks the service and slot rows, re-prices every
-- item from the database, and checks dough total, slot capacity, preorder
-- caps and per-item limits before inserting. Raises with a stable code in
-- the message ("CODE: human text") so the app can show a friendly error.
